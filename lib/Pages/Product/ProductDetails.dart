...
class _ProductDetails extends State<ProductDetails> {
                                ...
                                builder: (context, snapshot) {
                                  if (!snapshot.hasData) {
                                    ...
                                    return GridView.count(
                                        crossAxisCount: 2,
                                        children:
                                            getProductsSameCategory(snapshot));
                                  }
                                },
                              ),
                            ),
  ...
  getProductsSameCategory(AsyncSnapshot<QuerySnapshot> snapshot) {
    ...
    snapshot.data.docs.map((DocumentSnapshot doc) {
      if (doc["category"] == widget.category && widget.name != doc["name"]) {
        data.add(doc);
      }
    }).toList();
  ...
  getProductsSameBrand(AsyncSnapshot<QuerySnapshot> snapshot) {
    List<DocumentSnapshot> data = new List();
    snapshot.data.docs.map((DocumentSnapshot doc) {
      if (doc["brand"] == widget.brand && widget.name != doc["name"]) {
        data.add(doc);
      }
    }).toList();
    return data.map((DocumentSnapshot item) {
  ...
  _listedFavourite() async {
    ...
    if (isFavourite) {
      User _user = await FirebaseAuth.instance.currentUser;
      FirebaseFirestore.instance.collection("favourite").doc(favouriteId).set({
  ...
  _addProductToCart() async {
    ...
    if (widget.id != tempId) {
      setState(() {
        tempId = widget.id;
        widget.quantity = 1;
        FirebaseFirestore.instance.collection("carts").doc(widget.id).set({
          "cartsId": widget.id,
          "userEmail": _user.email,
          "products": {
            "productId": widget.id,
      ...
      setState(() {
        widget.quantity++;
        FirebaseFirestore.instance.collection("carts").doc(widget.id).update({
  ...
  getTempId() async {
    ...
    data.map((DocumentSnapshot doc) {
      if (doc["userEmail"] == _user.email && doc["cartsId"] == widget.id) {
        setState(() {
          tempId = widget.id;
          widget.quantity = doc["products"]["productQuantity"];
        });
      }
    }).toList();
  }
